...
class Behaviour:
    ...
    def run(self, output_mode):
        ...
        self.logger.info("Found configured markets: %s", self.tickers)

        market_data = self.exchange_interface.get_historical_data()

        self.logger.info("Fethced data for following ticker(s): %s", list(market_data.keys()))
    ...
    def _test_strategies(self, market_data, output_mode):
        ...
        for ticker in market_data:
            self.logger.info("Beginning analysis of %s", ticker)
            if ticker not in new_result:
                new_result[ticker] = dict()

            ohlcv_data = market_data[ticker]
            self.logger.info("Beginning analysis of %s", ticker)

            new_result[ticker]['indicators'] = self._get_indicator_results(
                ticker,
                ohlcv_data
            )

            new_result[ticker]['informants'] = self._get_informant_results(
                ticker,
                ohlcv_data
            )

            new_result[ticker]['crossovers'] = self._get_crossover_results(
                new_result[ticker]
            )

            if output_mode in self.output:
                output_data = deepcopy(new_result[ticker][ohlcv_data])
                print(
                    self.output[output_mode](output_data, ohlcv_data),
                    end=''
                )
            else:
                self.logger.warn()
    ...
    def _get_indicator_results(self, exchange, ohlcv_data):
        ...
        Args:
            ...
            ohlcv_data (Dataframe): OHLCV data for the ticker
    ...
    def _get_informant_results(self, exchange, market_pair):
        ...
        Args:
            exchange (str): The exchange to get the indicator results for.
            market_pair (Dataframe): The pair to get the market pair results for.
